...
TEST_F(CollectionTest, DesizeTest) {
    add_entries(5);
    ASSERT_EQ(collection->size(), 5);
    collection->resize(1);
    ASSERT_EQ(collection->size(), 1);
...
TEST_F(CollectionTest, ClearTest)
{
    add_entries(1);
    ASSERT_FALSE(collection->empty());
    collection->clear();
...
TEST_F(CollectionTest, PositiveIndexTest)
{
    EXPECT_THROW(collection->at(1), std::out_of_range);
}

// TODO: Create 2 unit tests of your own to test something on the collection - do 1 positive & 1 negative
// Tests that ten values can be added
...
// Test for negative indexes 
TEST_F(CollectionTest, NegativeIndexTest)
{
    EXPECT_THROW(collection->at(-1), std::out_of_range);
}