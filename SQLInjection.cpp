...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  //  something more generic
    ...
    // Added an if statement for injected "or" statements, so no manipulation can occur
    // Users must avoid using " or " in passwords/usernames for now with this solution
    if (sql.find(" or ") != std::string::npos) {
        std::cout << "Error. Potential injection. \n";
        return false;
    }
