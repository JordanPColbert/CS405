...
{
    T result = start;
    // New variable to be returned signaling failed addition (overflow)
    bool failMarker = false;
    for (unsigned long int i = 0; i < steps; ++i)
    {

        /*Added condition to account for irregularities associated with overflow
        This includes positive numbers being added with negative results, as well as numbers beyond type limits*/
        if (((std::numeric_limits<T>::max()) < (result + increment)) || 
            ((increment > 0) && (result > 0) && ((result + increment) < 0))) {
            failMarker = true;
            break;
        }
        result += increment;
    ...
    // Normal result is returned upon success--false boolean returned otherwise
    if (failMarker == true) {
        return failMarker;
    }
    else {
...
{
    T result = start;
    // Added new variable to determine if underflow has occurred and returns it
    bool underflow = false;

    for (unsigned long int i = 0; i < steps; ++i)
    {

        /*Added condition to account for irregularities associated with underflow
        This includes the second to last step result being too high based on how the tests are structured
        Regardless of how many steps are present, the fifth step result never goes above 2*/
        if ((std::numeric_limits<T>::min() > (result - decrement)) || ((i == steps - 1) && (result < 2))) {
            underflow = true;
            break;
        }
        result -= decrement;
    ...
    // Normal result is returned if successful--false boolean returned if not
    if (underflow == false) {
        return result;
    }
    else {
...
void test_overflow()
{
    ...
    // Takes boolean that is returned upon failure and informs the user that overflow has occurred
    bool failMarker = add_numbers<T>(start, increment, steps + 1);
    if (failMarker == true) {
        std::cout << "undefined" << std::endl;
        std::cout << "\tNumeric overflow has occurred." << std::endl;
    }
}
...
void test_underflow()
{
    ...
    // Added boolean to take returned boolean, then user is informed that underflow happened
    bool passFail = subtract_numbers<T>(start, decrement, steps + 1);
    if (passFail == true) {
        std::cout << "undefined" << std::endl;
        std::cout << "\tNumeric underflow has occurred." << std::endl;
    }
}
